...
from ubteacher.engine.trainer import BaselineTrainer, UBTeacherTrainer
...
def get_evaluator(cfg, dataset_name, output_folder=None):
    ...
    if evaluator_type == "lvis":
        return LVISEvaluatorMaxDets(dataset_name, cfg, True, output_folder, max_dets=[-1])
    elif evaluator_type == "coco":
        return COCOEvaluatorMaxDets(dataset_name, cfg, True, output_folder, max_dets=[-1])
    else:
        raise NotImplementedError


def _evaluate_predictions_on_lvis(lvis_gt, lvis_results, iou_type, max_dets=None, class_names=None):
    """
    ...
    lvis_results_obj = LVISResults(
        lvis_gt, lvis_results, max_dets_per_cat=10000, max_dets_per_im=-1
    )
...
def limit_mask_branch_proposals(model, max_proposals):
    ...
    def _forward_with_given_boxes_limited(self, features, instances):
        ...
        if max_instances > max_proposals and (self.mask_on or self.keypoint_on):
            ...
            for i in tqdm(range(0, max_instances, max_proposals)):
...
def inference_on_dataset(model, data_loader, evaluator, num_classes, topk, num_estimate, min_score):
    ...
    init_thresholds = torch.full((num_classes + 1,), fill_value=min_score, dtype=torch.float32).to(
        model.device
    )
    ...
    if comm.is_main_process():
        ...
        topk_preds = []
        for c, anns in by_cat.items():
            topk_preds.extend(sorted(anns, key=lambda a: a["score"], reverse=True)[: topk[c]])
        evaluator._predictions = [{"instances": topk_preds}]
    else:
    ...
    for dataset_name in cfg.DATASETS.TEST:
        ...
        if comm.is_main_process():
            for max_dets, max_dets_results in results_i.items():
                logger.info(
                    f"Evaluation results for {dataset_name},max_dets={max_dets} in " f"csv format:"
                )
